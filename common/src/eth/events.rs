...
fn decode_argument(
    ...
    let trimmed_data = match param.indexed {
        ...
        false => {
            data[64*param_index..64*(param_index + 1)].to_string()
        }
    };
    match param.param_type.as_str() {
        "address" => {
            Ok(trimmed_data[24..64].to_string())
        }
        "uint256" => {
...
pub async fn get_events(web3s: Web3<WebSocket>, params: serde_json::Value) -> anyhow::Result<Vec<DecodedEvent>> {
    // Execute query
    let res = web3s
        .transport()
